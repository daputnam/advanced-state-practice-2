...
class App extends Component {
  ...
  render() {
    ...
    let filteredCars = [];
    let currentMakeUppercase = null;
     if(this.state.currentMake !== null){
      this.state.currentMake.toUpperCase();
     } 
    if ((currentMakeUppercase === null) && (this.state.currentYear === null)) {
      filteredCars = carsForSale;
    } else if ((this.state.currentYear !== null) && (currentMakeUppercase !== null)) {
      filteredCars = carsForSale.filter((car) => {
        return ((car.year === this.state.currentYear) && (car.make === currentMakeUppercase));
      });
    } else if (this.state.currentYear !== null) {
      filteredCars = carsForSale.filter((car) => {
        return (car.year === this.state.currentYear)
      });
    } else if ((currentMakeUppercase !== null)) {
      filteredCars = carsForSale.filter((car) => {
        return (car.make === currentMakeUppercase);
      });
      // 
